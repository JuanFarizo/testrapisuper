...
const buyersController = require("../buyers/buyers")
  ...
  return catchAsync(async (req, res, next) => {
    ...
    try {
      await User.transaction(async (trx) => {
        ...
        if (type == "buyer") { // TODO: METER EN UNA TRANSACCION. PORQUE QUE PASA SI FALLA UNO DE ESTOS? EL USUARIO SE CREA IGUAL
          //console.log("CREANDO BUYER")
          buyerData = filterObj(
            req.body,
            'name',
            'surname',
          ...
          const newBuyer = await Buyers.query(trx).insert({
            userID: newUser.id,
            token: "" //TODO: VER QUE ONDA EL TOKEN
          });
          //console.log("BUYER CREADO")
          const newPersonalInfo = await BuyersPersonalInfo.query(trx).insert({
            buyerID: newBuyer.id,
            ...buyerData
          });
          //console.log("INFO CREADO")
          const newDirection = await BuyersAddresses.query(trx).insert({
            buyerID: newBuyer.id,
            ...addressData
          })
          //console.log("DIRECCION CREADO")


          newUserUpdated = await User.query(trx).findById(newUser.id).withGraphFetched("[buyer.[personalInfo, addresses.city]]")
        }
    ...
    await new Email(newUserUpdated, url).sendWelcome();

    createSendToken(newUserUpdated, 201, res);
  });
}
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  const freshUser = await User.query().findById(decoded.id).withGraphFetched("[buyer.[personalInfo, addresses.city]]")
