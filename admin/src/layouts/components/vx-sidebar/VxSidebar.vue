      ...
      <div @mouseenter="sidebarMouseEntered" @mouseleave="sidebarMouseLeave">
        <div class="header-sidebar flex items-end justify-between" slot="header">
          <div class="logo flex items-center">
            <img :src="logo" alt="logo" class="logoSidebar mr-4" v-if="logo">
            <span class="logo-text" v-show="isMouseEnter || !reduce" v-if="title">{{ title }}</span>
          </div>
          <div style="margin-bottom: 2px;">
            <template v-if="showCloseButton">
              <feather-icon style="color: #15B0CC;" icon="XIcon" class="m-0 cursor-pointer" @click="$store.commit('TOGGLE_IS_SIDEBAR_ACTIVE', false)"></feather-icon>
            </template>
            <template v-else-if="!showCloseButton && !sidebarItemsMin">
              <feather-icon style="color: #15B0CC;" icon="DiscIcon" class="mr-0 cursor-pointer" svg-classes="stroke-current" v-show="!reduce" @click="toggleReduce(true)" id="btnSidebarToggler"></feather-icon>
              <feather-icon style="color: #15B0CC;" icon="CircleIcon" class="mr-0 cursor-pointer" svg-classes="stroke-current" v-show="reduce" @click="toggleReduce(false)" id="btnSidebarToggler"></feather-icon>
            </template>
          </div>
        </div>
        ...
        <VuePerfectScrollbar ref="mainSidebarPs" class="scroll-area--main-sidebar pt-2" :settings="settings" @ps-scroll-y="psSectionScroll">
          <template v-for="(sidebarItem, index) in sidebarItems" v-if="!(sidebarItem.onlyMainHouse && !isMainHouse())">
            <!-- GROUP ITEM HEADER -->
...
<!-- - - - - - - - - - - - - - - - STYLE - - - - - - - - - - - - - - - -->
<style lang="scss">
@import "@/../../Dual/assets/scss/vuesax/components/vxSidebar.scss";
// Custom
.logoSidebar {
  max-width: 95%;
}
